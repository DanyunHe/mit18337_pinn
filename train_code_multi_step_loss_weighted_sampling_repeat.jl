...
for stepi in 1:dt_steps
        ...
        if stepi==1 #first timestepping, use initial data available, uniform sampling
                ...
                open("./loss_repeat/PINN_sample_x_u_loss_repeat_$(q)_$(dt)_$(stepi)_0.txt", "a") do file
                ...
                open("./loss_repeat/PINN_sample_x_u_loss_repeat_fullarray_$(q)_$(dt)_$(stepi)_0.txt", "a") do file
                    writedlm(file, [rand_array rand_array_u], ',')
                    flush(file)
                end
        ...
        current_loss_0=loss0()
        MSE_0=current_loss_0/(Ndata+2)
        append!(loss_array,current_loss_0)
        append!(MSE_array,MSE_0)
        append!(iteration_array,0)

        @save "./loss_repeat/PINN_NN_model_loss_repeat_$(q)_$(dt)_$(stepi)_0" NN
        open("./loss_repeat/PINN_iter_loss_MSE_loss_repeat_$(q)_$(dt)_$(stepi).txt", "a") do file
        ...
        for iteri in 1:number_big_step
                ...
                #every 1000 iterations, resample data points and redefine loss() based on loss weights
                if iteri%10==0 && iteri != 100 && stepi>1
# -loss weighted
                        #approximate of graident at the locations of the solutions
                        rand_array_cost=zeros(size(rand_array)[1])
                        for i in 1:size(rand_array)[1]
                                rand_array_cost[i]=sum(abs2,NN_U0([rand_array[i]]).-rand_array_u[i])
                        end
                        ...
                        sum_rand_array_cost=sum(rand_array_cost)
                        for i in 1:size(rand_array)[1]
                                rand_array_weight[i]=rand_array_cost[i]/sum_rand_array_cost
                        end

                        data_tn_weights=zeros(Ndata)
                        #the corresponding location x index of random uniform sampled data at time t0
                        ...
                        open("./loss_repeat/PINN_sample_x_u_loss_repeat_$(q)_$(dt)_$(stepi)_$(total_iteration_i).txt", "a") do file
                            writedlm(file, [data_tn_x data_tn_u], ',')
                            flush(file)
                        end
                end
                ...
                @save "./loss_repeat/PINN_NN_model_loss_repeat_$(q)_$(dt)_$(stepi)_$(total_iteration_i)" NN
        ...
        #save error to file
        open("./loss_repeat/losss/PINN_error_loss_repeat.txt", "a") do file
        ...
        savefig(img,"./loss_repeat/PINN_soln_plot_loss_repeat_$(q)_$(dt)_$(stepi).png")
